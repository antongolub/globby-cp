...
describe('JS API', () => {
  test('requires both `from` and `to`', async (t) => {
    await assert.rejects(() => copy({from: 'foo'}),/required/)
    await assert.rejects(() => copy({to: 'bar'}),/required/)
  })

  test('`to` cannot be pattern', async (t) => {
    await assert.rejects(() => copy({to: '*bar', from: 'foo'}),/pattern/)
  })

  test('copy() abs to abs', async (t) => {
  ...
  test('copy() file to file', async (t) => {
    const temp = tempy.temporaryDirectory()
